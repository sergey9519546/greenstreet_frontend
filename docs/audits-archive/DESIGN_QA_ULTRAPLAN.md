...
| ~~P0-4~~ | ~~Home DOM bleeds → 2 `<h1>`~~ **CORRECTED: already mitigated.** App.tsx toggles the Webflow home (`#webflow-root`) to `display:none` + `hidden` + `inert` + `aria-hidden=true` + `pointer-events:none` on every non-marketing route (verified in dev **and** prod `dist`). The 2nd h1 is fully removed from layout/a11y/interaction; the visible page has exactly 1 h1. Not a real defect. | — | **No action** (a shell-split would be risk for no gain). *Optional:* gate GTM/LinkedIn/ads to marketing-only **iff** you don't want analytics on app routes (likely you do). |
| ~~P0-5~~ → P2 | `/deal-analyzer` resolves to `/lender-intel` (intentional alias) but CTAs/nav imply a separate tool | `router/resolve.ts` + "Deal Analyzer" CTAs | Relabel/remove the "Deal Analyzer" CTAs (DealAnalyzerPage.tsx is dead code). Cosmetic, not P0. |
| ~~P0-6~~ | ~~Lead form captures nothing~~ **FALSE ALARM — fixed/clarified.** Leads persist to Firestore `leads` (`QualifyModal.tsx`); the "TODO" was a CRM-sync note. | — | Done (comment clarified). |
