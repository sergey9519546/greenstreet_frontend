...
function getGeminiClient(): GoogleGenAI {
  if (!aiClient) {
    ...
    if (!key) {
      console.warn("WARNING: GEMINI_API_KEY is not defined in the environment. AI features will fail with explicit error responses.");
    }
...
app.post("/api/compliance/review", async (req, res) => {
  try {
    ...
    if (!content) {
      return res.status(400).json({ error: "Content field is required for compliance review." });
    }

    const ai = getGeminiClient();
    if (!process.env.GEMINI_API_KEY) {
      return res.status(503).json({
        error: "GEMINI_API_KEY is missing. Please add your Gemini API Key in the Settings > Secrets configuration panel to use this feature."
      });
    }
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        responseSchema: {
          type: Type.OBJECT,
          required: ["promissoryClaims", "omissionsOfRisk", "prominentPraise", "requiredDisclosures", "complianceScore", "overallSummary"],
          properties: {
            complianceScore: {
              type: Type.INTEGER,
              description: "Overall safety score from 0 (extremely non-compliant) to 100 (fully compliant)."
            },
            overallSummary: {
              type: Type.STRING,
              description: "Brief professional summary of the marketing copy from a compliance perspective."
            },
            promissoryClaims: {
              type: Type.ARRAY,
              items: {
                type: Type.OBJECT,
                required: ["textSegment", "explanation", "severity", "suggestedRewrite"],
                properties: {
                  textSegment: { type: Type.STRING, description: "The violating phrase or sentence." },
                  explanation: { type: Type.STRING, description: "Why this violates SEC/FINRA rules." },
                  severity: { type: Type.STRING, description: "HIGH, MEDIUM, or LOW." },
                  suggestedRewrite: { type: Type.STRING, description: "A compliant alternative phrase." }
                }
              }
            },
            omissionsOfRisk: {
              ...
              items: {
                ...
                properties: {
                  explanation: { type: Type.STRING, description: "Why risk warning is omitted here." },
                  suggestedDisclaimer: { type: Type.STRING, description: "Specific disclaimer copy that must be appended." }
                }
              }
            },
            prominentPraise: {
              ...
              items: {
                ...
                properties: {
                  textSegment: { type: Type.STRING, description: "The testimonial, award, or past-performance claim." },
                  explanation: { type: Type.STRING, description: "Requirement violation description." },
                  suggestedFix: { type: Type.STRING, description: "How to balance this praise (e.g. adding dates, criteria)." }
                }
              }
            },
            requiredDisclosures: {
              type: Type.ARRAY,
              description: "A list of standard legal disclaimers recommended based on the file content.",
              items: {
                type: Type.STRING
              }
            }
    ...
    return res.status(500).json({ error: error.message || "Internal compliance engine failure." });
  }
});
...
app.post("/api/compliance/sec-search", async (req, res) => {
  try {
    ...
    if (!process.env.GEMINI_API_KEY) {
      return res.status(503).json({
        error: "GEMINI_API_KEY is missing. Please configure your API key to enable Search Grounding."
      });
    }
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        systemInstruction: "You are an AI Compliance Counsel. Provide precise details, rule references, dates, and citation labels. State points clearly with markdown.",
        tools: [{ googleSearch: {} }]
      }
    });
    ...
    return res.status(500).json({ error: error.message || "Failed to complete search grounding query." });
  }
});
...
app.post("/api/compliance/jurisdiction-maps", async (req, res) => {
  try {
    ...
    if (!process.env.GEMINI_API_KEY) {
      return res.status(503).json({
        error: "GEMINI_API_KEY is missing. Please configure your key in Secrets."
      });
    }
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        systemInstruction: "Identify official geographic regulatory bodies, regional office details, addresses, and important Blue Sky rule codes.",
    ...
    return res.status(500).json({ error: error.message || "Failed to process location compliance details." });
  }
});
...
app.post("/api/compliance/think-guide", async (req, res) => {
  try {
    ...
    if (!process.env.GEMINI_API_KEY) {
      return res.status(503).json({
        error: "GEMINI_API_KEY is missing. High Thinking reasoning cannot progress without a valid API key."
      });
    }
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        systemInstruction: "You are the Principal Regulatory Analyst. Deconstruct the inquiry. Outline historical precedents, SEC Risk Alerts, and comprehensive enforcement audit points.",
        thinkingConfig: {
    ...
    return res.status(500).json({ error: error.message || "Failed to execute High Thinking model generation." });
  }
});
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`Server successfully started. Running full-stack on http://0.0.0.0:${PORT}`);
  });
}
