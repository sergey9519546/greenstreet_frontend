...
// Vercel deployment: api/_app.cjs is now tracked in git (commit e79efaf) and
// deployed alongside api/index.js. The obsolete functions.includeFiles declaration
// was removed from vercel.json. Verify the bundle exists on disk.
for (const artifact of ['api/_app.cjs', 'dist/engineWorker.cjs']) {
  const artifactPath = atRoot(artifact);
  assert.ok(existsSync(artifactPath), `Vercel deployment requires ${artifact} to exist (generated by build)`);
}
assert.ok(
