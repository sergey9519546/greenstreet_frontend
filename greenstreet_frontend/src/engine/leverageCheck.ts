...
// ============================================================
// LTV CONSTRAINT CHECK — portfolio leverage check
// ============================================================
...
export function computeLeverageCheck(
  ...
  for (const prop of properties) {
    ...
    totalBalance += prop.balance;

    if (prop.ltv > targetMaxLTV) {
      violations.push({
        id: prop.id,
        name: prop.name,
        ltv: prop.ltv,
        excess: prop.ltv - targetMaxLTV,
      });
    }
  }

  const portfolioLTV = totalValue > 0 ? (totalBalance / totalValue) * 100 : 0;
  const passes = portfolioLTV <= targetMaxLTV && violations.length === 0;
