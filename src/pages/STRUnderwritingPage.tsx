      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: "40px", alignItems: "start" }}>
        ...
        <div>
          {!result ? (
            ...
            <>
              <div style={{ ...card, borderColor: result.legality.status === "CLEAR" ? MINT : result.legality.status === "RESTRICTED" ? YELLOW : result.legality.status === "UNCERTAIN" ? YELLOW : "#ff6b6b" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "12px", marginTop: "20px" }}>
                {[
                  ...
                  <div key={w.label} style={{ background: "rgba(0,55,56,0.04)", borderRadius: "10px", padding: "16px", border: w.label.includes(result.underwriting.bestWorld) ? `2px solid ${MINT}` : "1px solid rgba(0,55,56,0.1)" }}>
                    <div style={{ fontSize: "11px", color: "#888", letterSpacing: "0.1em", textTransform: "uppercase", fontWeight: 700 }}>{w.label}</div>
                    <div style={{ fontSize: "28px", fontWeight: 800, color: w.value >= 1.25 ? MINT : w.value >= 1.0 ? YELLOW : "#ff6b6b", marginTop: "6px" }}>{w.value.toFixed(2)}x</div>
                    <div style={{ fontSize: "11px", color: "#888", marginTop: "4px" }}>Qualifying Rent: ${Math.round(w.rent).toLocaleString()}</div>
                  </div>
                ))}
              </div>

              <div style={{ ...card, marginTop: "20px", background: "rgba(77,189,151,0.08)", border: `1px solid ${MINT}` }}>
                ...
                <div style={{ fontSize: "12px", color: "#aaa", marginTop: "6px" }}>Haircut Applied: <strong style={{ color: CREAM }}>{(result.underwriting.haircutPercent * 100).toFixed(1)}%</strong></div>
                <div style={{ fontSize: "12px", color: "#aaa", marginTop: "4px" }}>Best Qualifying Rent: <strong style={{ color: CREAM }}>${Math.round(result.underwriting.bestQualifyingRent).toLocaleString()}/mo</strong></div>
              </div>
                  ...
                  {result.seasonality.map((m, i) => {
                    ...
                    return (
                      <div key={i} style={{ background: "rgba(0,55,56,0.04)", borderRadius: "6px", padding: "8px 4px", textAlign: "center" }}>
                        ...
                        <div style={{ fontSize: "11px", color: CREAM, fontWeight: 700 }}>${Math.round(m.monthlyRent).toLocaleString()}</div>
                      </div>
                    );
                  })}
                </div>
