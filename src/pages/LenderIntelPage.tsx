...
export default function LenderIntelPage({ onBack, onNavigate }: { onBack?: () => void; onNavigate?: (v: any) => void }) {
  ...
  const selectedPrograms = scored.filter(p => selectedForCompare.has(p.id));
        ...
        <div style={{ maxWidth: dc.maxW, margin: "0 auto" }}>
          <div id="gs-hero-content" className="li-hero-grid dc-hero" style={{ display: "grid", gridTemplateColumns: "1.1fr 0.9fr", gap: "clamp(32px,5vw,72px)", alignItems: "center" }}>
            ...
            <div style={{ background: "rgba(238,239,211,0.06)", borderRadius: 16, border: "1px solid rgba(238,239,211,0.12)", padding: "clamp(20px,2.4vw,32px)" }}>
              ...
              <div style={{ background: matchCount > 0 ? "rgba(77,189,151,0.12)" : risk.dangerBg, border: `1px solid ${matchCount > 0 ? dc.emerald : risk.danger}`, borderRadius: radius.sm, padding: "12px 16px", textAlign: "center" }}>
                <Mono style={{ fontSize: 32, fontWeight: 700, color: matchCount > 0 ? dc.emerald : risk.danger, lineHeight: 1 }}>{matchCount}</Mono>
          ...
          >
                  ...
                  >
                    ...
                    <div>
                      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 6 }}>
                        ...
                        <span
                          style={{
                            ...
                            color: statusColor,
                            background: p.fits ? "rgba(77,189,151,0.16)" : p.passed >= 3 ? risk.cautionBg : risk.dangerBg,
                            borderRadius: radius.sm,
                            padding: "3px 9px",
                          }}
                        >
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                        {p.checks.map((c) => (
                          <span
                            ...
                            style={{
                              ...
                              fontWeight: 600,
                              fontFamily: dc.mono,
                              color: c.ok ? dc.emerald : risk.danger,
                              background: c.ok ? "rgba(77,189,151,0.12)" : risk.dangerBg,
                              borderRadius: 5,
                              padding: "4px 8px",
                              letterSpacing: "-0.01em",
                            }}
                        ...
                        {/* Specialty flags */}
                        {p.noRatio && (
                          <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11, fontWeight: 600, fontFamily: dc.mono, color: swatch.lemon, background: risk.cautionBg, borderRadius: radius.sm, padding: "4px 8px" }}>
                            ★ no-ratio
                          </span>
                        )}
                        {p.isSTR && (
                    ...
                    <div style={{ textAlign: "right", minWidth: 56 }}>
                      ...
                      <button
                        ...
                        style={{
                          ...
                          textTransform: "uppercase",
                          background: selectedForCompare.has(p.id) ? swatch.lemon : "rgba(238,239,211,0.1)",
                          color: selectedForCompare.has(p.id) ? swatch.midnight : "rgba(238,239,211,0.72)",
                          border: `1px solid ${selectedForCompare.has(p.id) ? swatch.lemon : "rgba(238,239,211,0.3)"}`,
                          borderRadius: radius.sm,
                          cursor: (!selectedForCompare.has(p.id) && selectedForCompare.size >= 3) ? "not-allowed" : "pointer",
                          opacity: (!selectedForCompare.has(p.id) && selectedForCompare.size >= 3) ? 0.4 : 1,
                          transition: "all .15s",
                        }}
                      >
                        {selectedForCompare.has(p.id) ? "✓ Compare" : "+ Compare"}
                      </button>
                    </div>
                  </div>
                );
            ...
            {selectedForCompare.size > 0 && (
              <div style={{
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", color: swatch.lemon, marginBottom: 4 }}>
                    Compare Programs
                  </div>
                  <div style={{ fontSize: 13, color: dc.cream }}>
                    {selectedForCompare.size} selected {selectedForCompare.size < 2 && "(select at least 2)"}
                  </div>
                </div>
                <button
            ...
            {selectedPrograms.length >= 2 && (
              <div style={{ marginTop: 24, background: swatch.midnight, border: `2px solid ${swatch.lemon}`, borderRadius: radius.md, padding: 24, overflow: "auto" }}>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  <thead>
                    <tr style={{ borderBottom: "1px solid rgba(238,239,211,0.2)" }}>
                      <th style={{ textAlign: "left", padding: "10px 12px", fontSize: 11, fontWeight: 700, letterSpacing: "0.05em", textTransform: "uppercase", color: "rgba(238,239,211,0.62)" }}>
                        Program
                      </th>
                      {selectedPrograms.map(p => (
                        <th key={p.id} style={{ textAlign: "left", padding: "10px 12px", fontWeight: 700, color: p.fits ? dc.emerald : risk.danger }}>
                          {p.name}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {[
                      { label: "Status", key: (p: typeof selectedPrograms[0]) => p.fits ? "FITS" : "NO FIT" },
                      { label: "Min FICO", key: (p: typeof selectedPrograms[0]) => p.minFICO.toString() },
                      { label: "Max LTV", key: (p: typeof selectedPrograms[0]) => `${p.effectiveLTV ?? p.maxLTV}%` },
                      { label: "DSCR Floor", key: (p: typeof selectedPrograms[0]) => p.noRatio ? "No-ratio OK" : `${p.dscrFloor.toFixed(2)}` },
                      { label: "Max Loan", key: (p: typeof selectedPrograms[0]) => fmtLoan(p.maxLoan) },
                      { label: "STR", key: (p: typeof selectedPrograms[0]) => p.isSTR ? "✓" : "—" },
                      { label: "5+ Units", key: (p: typeof selectedPrograms[0]) => p.multiFamily ? "✓" : "—" },
                      { label: "Foreign National", key: (p: typeof selectedPrograms[0]) => p.nonUsInvestor ? "✓" : "—" },
                      { label: "Fit Score", key: (p: typeof selectedPrograms[0]) => Math.round(p.score).toString() },
                    ].map(row => (
                      <tr key={row.label} style={{ borderBottom: "1px solid rgba(238,239,211,0.08)" }}>
                        <td style={{ padding: "10px 12px", fontWeight: 600, color: "rgba(238,239,211,0.72)" }}>
                          {row.label}
                        </td>
                        {selectedPrograms.map(p => (
                          <td key={p.id} style={{ padding: "10px 12px", color: dc.cream, fontFamily: dc.mono }}>
                            {row.key(p)}
                          </td>
                        ))}
                      </tr>
