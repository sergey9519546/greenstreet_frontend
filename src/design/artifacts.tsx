...
/* The meter claims a fixed width so it survives a greedy \`1fr auto\` grid track,
   but below the tablet break it goes fluid rather than push such a row wider
...
export function ClaudeDscrGauge({
  ...
  // Axis labels are laid out against the width the meter ACTUALLY got, not the
  // `size` it asked for — consumers can and do squeeze it.
  ...
  // A threshold that is drawn but whose label collides with its neighbour is no
  // better than an unlabelled band, so the axis is LAID OUT, not just emitted.
  // Widths are estimated from the real rendered box (mono advance ≈ 0.60em,
  // uppercase ≈ 0.63em + tracking): a name that would touch the previous one
  // drops to the next line, and if the scale is too tight to seat the threshold
  // numbers at all, the labels are dropped rather than overlapped — the ticks
  // and the sentence below still carry the meaning.
  const W = boxW || size;
  const capLine = Math.round(capSize * 1.35);
  const numW = 4 * (capSize + 1) * 0.6;
  const nameW = (t: string) => t.length * capSize * 0.67;
  const anchors = [0, ...marks.map((m) => posOf(m.at)), 1];
  const tightest = Math.min(...anchors.slice(1).map((a, i) => (a - anchors[i]) * W));
  const showMarkLabels = tightest >= numW + 6;

  let prevRight = -Infinity;
  let row = 0;
  const laidOut = (showMarkLabels ? marks : []).map((m) => {
    const w = nameW(m.name);
    const left = posOf(m.at) * W - w / 2;
    row = left < prevRight + 6 ? row + 1 : 0;
    prevRight = Math.max(prevRight, left + w);
    return { ...m, row };
  });
  const capHeight = capLine * (laidOut.length ? 2 + Math.max(...laidOut.map((m) => m.row)) : 1);

  const capNumber = (left: string, text: string, transform?: string) => (
    <div style={{ position: "absolute", top: 0, left, transform, fontFamily: font.mono, fontSize: capSize + 1, fontWeight: 600, color: onDark.dim, lineHeight: `${capLine}px`, whiteSpace: "nowrap" as const }}>
      {text}
    </div>
  );
  ...
  return (
    ...
    <div
      ref={rootRef}
      className="gsa-meter-root"
      style={{ width: size, maxWidth: "100%", ["--gsa-meter-w" as string]: `${size}px` } as React.CSSProperties}
    >
      ...
      <div style={{ position: "relative", height: capHeight, marginTop: 2 }}>
        {capNumber("0", min.toFixed(2))}
        {capNumber("100%", max.toFixed(2), "translateX(-100%)")}
        {laidOut.map((m) => (
          <React.Fragment key={m.name}>
            {capNumber(`${posOf(m.at) * 100}%`, m.at.toFixed(2), "translateX(-50%)")}
            <div
              style={{
                position: "absolute",
                top: capLine * (1 + m.row),
