...
// Explicit domain ceilings keep browser-valid scientific notation and oversized
// API payloads from reaching financial math. Values above a ceiling are rejected
// at the input boundary rather than allowed to overflow a later calculation.
...
export interface DealRequest {
  ...
  hoa?: number;               // monthly
  /** Annual flood-insurance premium in dollars; PITIA converts it to monthly once. */
...
function abbrevState(state: string): string {
  const normalized = (state || '').trim().toUpperCase().replace(/\./g, '').replace(/\s+/g, ' ');
...
export function buildEngineInputs(req: DealRequest): EngineInputs {
  ...
  const property: PropertyInputs = {
    ...
    floodInsurance: nonNegative(req.floodInsurance, 0, MAX_ANNUAL_PROPERTY_EXPENSE),
